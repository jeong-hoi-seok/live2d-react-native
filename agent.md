...
3. `docs/ai-wiki-guide.md`를 확인합니다.
4. `wiki/index.md`가 있으면 먼저 읽고, 작업 주제와 관련된 wiki 문서를 확인합니다.
5. 기존 코드와 설정 파일이 있으면 먼저 읽고, 같은 패턴으로 작업합니다.
6. 새 의존성 추가 전 현재 설치된 라이브러리와 Expo SDK 54 기본 제공 기능으로 해결 가능한지 확인합니다.
7. 네이티브 설정 변경이 필요하면 Expo managed workflow와 EAS 설정 영향을 먼저 확인합니다.
...
- AI 작업 지식과 재사용 가능한 구현 맥락은 `wiki`에 둡니다.

## AI Wiki 규칙

자세한 규칙은 `docs/ai-wiki-guide.md`를 따릅니다.

- AI Wiki는 AI 에이전트가 작업을 수행하기 위해 참조하고, 작업 중 발견한 지식을 기록하는 전용 문서 시스템입니다.
- 모든 AI Wiki 문서는 `wiki/` 디렉토리에 작성합니다.
- 작업 시작 시 `wiki/index.md`가 있으면 먼저 확인합니다.
- 작업 주제와 관련된 wiki 문서가 있으면 코드 수정 전에 읽습니다.
- 반복 참조할 가능성이 큰 구조, 제약, 시행착오, 호환성 정보는 wiki에 기록합니다.
- 기존 주제 문서가 있으면 새 문서 대신 기존 문서를 갱신합니다.
- wiki를 추가하거나 수정했다면 결과 보고에 문서 경로를 포함합니다.
- 비밀 키, 토큰, 인증 정보, 개인 계정 정보는 wiki에 기록하지 않습니다.
...
- 작업 전 필요한 wiki 문서를 확인합니다.
- 작업 중 새로 확인한 재사용 가능 지식은 wiki에 기록하거나 갱신합니다.
- 사용자 변경분을 되돌리지 않습니다.
